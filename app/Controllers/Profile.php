...
class Profile extends AdminBaseController
{
	...
	public function updateProfile()
	{
		...
		return redirect()->to('profile/')->with('notifySuccess', 'Profile has been Updated Successfully');
	...
	public function updatePassword()
	{
		...
		if ( post('password') !== post('password_confirm') ) {
			return redirect()->to('profile/')->with('notifyError', 'Password does not matches with Confirm Password !');
		}
		
		if ( strlen(post('password')) < 8 ) {
			return redirect()->to('profile/index/change_password')->with('notifyError', 'Password must have atleast 8 Characters');
		}
		...
		if ( hash('sha256', post('old_password')) != (new UserModel)->getRowById($id, 'password') ) {
			...
			return redirect()->to('profile/')->with('notifyError', 'Invalid Old Password !');
		}
	...
	public function updateProfilePic()
	{
		if (! $this->validate([
			...
			return redirect()->to('profile/')->with('notifyError', $this->validator->getErrors()['image']);
        }
		...
		if (!empty($_FILES['image']['name'])) {
			...
			if(!$upload){
				return redirect()->to('profile/')->with('notifyError', 'Server Error Occured while Uploading Image !');
			}
			...
			return redirect()->to('profile/')->with('notifySuccess', 'Profile Image has been Updated Successfully');
